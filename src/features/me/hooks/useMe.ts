...
import type { LoansListResponse } from "@/types/Loan";
...
export function useMe() {
  return useQuery<ProfileResponse, Error>({
    queryKey: ["me"],
    queryFn: async () => {
      const res: ApiResponse<ProfileResponse> = await getMe();
...
export function useMeLoans(
  ...
  return useQuery<LoansListResponse, Error>({
    ...
    queryFn: async () => {
      const res: ApiResponse<LoansListResponse> = await getMeLoans(
        status,
        page,
        limit
      );
...
export function useMeReview(page: number = 1, limit: number = 10) {
  return useQuery<ReviewListResponse, Error>({
    queryKey: ["books", page, limit],
    queryFn: async () => {
      const res: ApiResponse<ReviewListResponse> = await getMeReview(
        page,
        limit
      );
